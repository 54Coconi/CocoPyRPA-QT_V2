...
from PyQt5.QtCore import Qt, qAbs, QRect, QDateTime, QMimeData, pyqtSignal  # 用于处理核心的常量，函数和类
from PyQt5.QtGui import QPen, QPainter, QColor, QGuiApplication, QIcon  # 用于处理图形，绘画，颜色和屏幕相关的功能
from PyQt5.QtWidgets import QApplication, QWidget, QDialog, QVBoxLayout, QGraphicsView, QGraphicsScene, \
    QGraphicsPixmapItem, QPushButton, QMessageBox, QHBoxLayout  # 用于创建应用程序和窗口

import resources_rc

# 背景颜色
BACKGROUND_COLOR = [
    QColor(0, 0, 0, 100),       # 黑色半透明
    QColor(255, 255, 255, 50)   # 白色半透明
]

# 画笔颜色
PEN_COLOR = [
    QColor(113, 214, 82, 255),  # 绿色
    QColor(249, 42, 173, 100)   # 粉红色半透明
]
...
class CaptureScreen(QWidget):
    ...
    save_image_path = 'work/work_images/'

    screen_shot_finish_signal = pyqtSignal(str)  # 截图完成的信号
    screen_window_close_signal = pyqtSignal()  # 窗口关闭的信号
    ...
    def closeEvent(self, a0: QtGui.QCloseEvent) -> None:
        """ 重写关闭事件 """
        self.screen_window_close_signal.emit()
    ...
    def paintBackgroundImage(self):
        ...
        绘制模糊背景图的方法
        """
        shadowColor = BACKGROUND_COLOR[1]  # 黑色半透明
    ...
    def paintEvent(self, event):
        ...
        self.paintBackgroundImage()  # 绘制模糊背景图
        penColor = PEN_COLOR[0]  # 画笔颜色
        self.painter.setPen(QPen(penColor, 2, Qt.DashLine, Qt.RoundCap))  # 设置画笔,绿色,1px大小,实线,圆形笔帽
        if self.isMousePressLeft is True:  # 如果鼠标按下了左键
    ...
    def showImagePreview(self):
        ...
        dialog.setWindowIcon(QIcon(":/icons/logo"))
    ...
    def saveImage(self, dialog):
        ...
        # 获取目录路径
        directory = os.path.dirname(self.save_image_path)
        # 检查目录是否存在，如果不存在则创建
        ...
        try:
            ...
            clipboard.setMimeData(mime_data)
            # TODO: 发送 screen_shot_finish_signal 信号
            self.screen_shot_finish_signal.emit(file_name)
            QMessageBox.information(dialog, "提示", "图片路径已保存到粘贴板, 按ESC键退出", QMessageBox.Ok)
