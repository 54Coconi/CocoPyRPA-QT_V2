...
class ScreenCoorWidget(QWidget):
    ...
    coordinateChanged = pyqtSignal(tuple)  # 定点坐标改变信号
    ...
    def __init__(self, functype):
        ...
        # TODO: 创建一个QPixmap对象，加载自定义的鼠标指针样式图像(使用resources_rc资源文件)
        pixmap = QPixmap(':/icons/screen-coordinate')
        # 创建一个自定义的鼠标样式
...
if __name__ == '__main__':
    def tes_class_ScreenCoorWidget(functype):
        """ 测试模块功能 """
        app = QApplication(sys.argv)  # 创建一个QApplication对象

        capture_widget = ScreenCoorWidget(functype)  # 创建一个ScreenCoorWidget对象（1 鼠标定点移动 2 鼠标相对移动）
